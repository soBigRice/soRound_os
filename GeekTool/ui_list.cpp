...
/* 滚动时:每行按到中心距离做 translate_x + 淡出(便宜又稳,勿用 transform_zoom) */
...
lv_obj_t *ui_list_create(lv_obj_t *parent) {
  ...
  lv_obj_set_style_pad_top(list, LCD_HEIGHT / 2 - ROW_H / 2, 0);
  lv_obj_set_style_pad_bottom(list, LCD_HEIGHT / 2 - ROW_H / 2, 0);
...
lv_obj_t *ui_list_row_right(lv_obj_t *row) {
  return (lv_obj_get_child_cnt(row) >= 2) ? lv_obj_get_child(row, 1) : NULL;
}
