...
static void settings_enter(lv_obj_t *parent) {
    ...
    // 可滚动单页:设置项在上,about(徽标/版本/设备)在下,往下滑即见 —— 合并原独立 about app
    ...
    char b[8];
    // 设置项(y ~96..404,收进圆内接区;位置不变)
    mk_row(page, "brightness", 96, SETTINGS_BRIGHT_MIN, 0xFF, settings_brightness(), br_changed, &g_br_val);
    snprintf(b, sizeof b, "%d%%", settings_brightness() * 100 / 255);
    lv_label_set_text(g_br_val, b);

    lv_obj_t *vsl = mk_row(page, "volume", 186, 0, 100, settings_volume(), vol_changed, &g_vol_val);
    ...
    mk_face_btn(page, LV_SYMBOL_LEFT,  -264, 268, -1);
    ...
    mk_face_btn(page, LV_SYMBOL_RIGHT, -92, 268, +1);
    ...
    // ===== about 段(原独立 about app 合并进来,往下滑可见)=====
    glyph_line(page, 116, 452, 350, 452, 12, 2, COL_TXT2);  // 点描分隔线

    int ly = 528;                                           // 同心点描环徽标(品牌:圆)+ 红核
    glyph_circle(page, 233, ly, 44, 13, 3, COL_TXT);
    glyph_circle(page, 233, ly, 28, 12, 3, COL_TXT);
    ...
    lv_obj_align(row, LV_ALIGN_TOP_MID, 0, 588);
    ...
    lv_obj_align(ver, LV_ALIGN_TOP_MID, 0, 634);
    ...
    lv_obj_align(dev, LV_ALIGN_TOP_MID, 0, 662);
}
