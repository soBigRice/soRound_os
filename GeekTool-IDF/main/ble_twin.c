...
static int gap_event(struct ble_gap_event *ev, void *arg) {
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        s_notify_on = false;
        start_advertising();                 // 断开后继续广播,等下次连接
        return 0;
...
static void host_task(void *param) {
    (void)param;
    nimble_port_run();                       // nimble_port_stop() 后返回
...
bool ble_twin_start(void) {
    ...
    if (e != ESP_OK) { ESP_LOGE(TAG, "nimble_port_init: %d", e); return false; }
...
void ble_twin_stop(void) {
    ...
    int rc = nimble_port_stop();             // 停 host 任务(host_task 里会 freertos_deinit)
...
bool ble_twin_connected(void) { return s_conn != BLE_HS_CONN_HANDLE_NONE; }

bool ble_twin_notify(const uint8_t *data, size_t len) {
    if (!s_running || s_conn == BLE_HS_CONN_HANDLE_NONE || !s_notify_on) return false;
